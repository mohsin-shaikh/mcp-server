...
export const httpModule: McpModule = {
  ...
  register(server, ctx) {
    server.registerTool(
      ...
      async (args) => {
        logToolCall(ctx, "http_fetch", { method: args.method, url: args.url });

        if (!ctx.isHostAllowed(args.url)) {
          logToolResult(ctx, "http_fetch", false, { reason: "host_denied" });
        ...
        try {
          ...
          logToolResult(ctx, "http_fetch", res.ok, { status: res.status });
          if (!res.ok) {
          ...
          if (err instanceof Error && err.name === "TimeoutError") {
            logToolResult(ctx, "http_fetch", false, { reason: "timeout" });
            return toolError(formatTimeoutError(ctx.config.httpTimeoutMs));
          }
          logToolResult(ctx, "http_fetch", false);
